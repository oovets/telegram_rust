...
async fn run_app<B: ratatui::backend::Backend>(
    ...
    loop {
        ...
        // Show the terminal cursor only when typing in the input
        if app.show_help || app.focus_on_chat_list || app.inline_preview_path.is_some() {
                    ...
                    match key.code {
                        ...
                        KeyCode::Esc => {
                            if app.pending_delete.take().is_some() {
                                app.notify("Delete cancelled");
                            } else if app
                                .panes
                                .get(app.focused_pane_idx)
                                .is_some_and(|p| p.search_active)
                            {
                                app.restore_search(app.focused_pane_idx);
                            } else if let Some(pane) = app
                                .panes
                                .get_mut(app.focused_pane_idx)
                                .filter(|p| p.reply_to_message.is_some())
                            {
                                pane.reply_to_message = None;
                                pane.hide_reply_preview();
                            }
                        }
                        KeyCode::Char('y') | KeyCode::Char('Y') if app.pending_delete.is_some() => {
                            if let Some(pd) = app.pending_delete.take() {
                                app.queue_delete_message(
                                    pd.pane_idx,
                                    pd.chat_id,
                                    pd.actual_msg_id,
                                    pd.msg_num,
                        ...
                        KeyCode::Char('n') | KeyCode::Char('N') if app.pending_delete.is_some() => {
                        ...
                        KeyCode::BackTab => {
                            let input_empty = app
                                .panes
                                .get(app.focused_pane_idx)
                                .is_none_or(|p| p.input_buffer.is_empty());
                ...
                Event::Mouse(mouse) => {
                    ...
                    match mouse.kind {
                        ...
                        MouseEventKind::Down(MouseButton::Left) => {
                            ...
                            {
                                app.handle_chat_list_click(mouse.row, area);
                            }
                            app.handle_mouse_click(mouse.column, mouse.row);
                            app.load_pane_messages_if_needed(app.focused_pane_idx);
                        }
                        _ => {}
                    }
                }
